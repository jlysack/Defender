...
def init_rad_control_logger(debug_enabled=False):
    ...
    file_handler = logging.FileHandler(
        filename=str("logs/" + app_name + "_" + now + ".log")
    )
...
def plot_heat_map(normalized_amp, sigpro_cfg):
    if sigpro_cfg.tactical_mode:
        # plt.imshow(normalized_amp, cmap='hot', interpolation='none', \
        plt.imshow(
            normalized_amp,
            interpolation="none",
            extent=[
                sigpro_cfg.range_extent_vec[0],
...
def compute_sum_data(rp_iq_data, sigpro_cfg):
    ...
    amplitude_map_iq = np.fft.fftshift(
        np.fft.fft(rp_iq_data * sigpro_cfg.ant_window_2d, sigpro_cfg.NFFTAnt, axis=1)
        / sigpro_cfg.sca_ant_window,
        axes=1,
    )
    ...
    normalized_amp = (
        amplitude_map_db - amp_max
    )  # subtract instead of divide since it's log math
...
def compute_range_and_angle(normalized_amp, sigpro_cfg):
    ...
    if no_detection_check(normalized_amp, sigpro_cfg):
...
def check_engagement_zone(range_m, angle_deg, sigpro_cfg):
    if (
        range_m > (sigpro_cfg.min_range + feet_to_m(const.ENGAGE_ZONE_RANGE_OFFSET))
        and range_m < (sigpro_cfg.max_range - feet_to_m(const.ENGAGE_ZONE_RANGE_OFFSET))
        and np.abs(angle_deg) < const.ENGAGE_ZONE_ANGLE_LIMIT
    ):
        return True
...
def radar_search(Brd, sigpro_cfg, plot_cfg, process_queue):
    ...
    while True:
        ...
        if check_radar_safety_file() == "0":
            logger.warn(
                f"Function radar_control.check_radar_safety_file() returned 0. Radiation disabled, exiting radar_search loop."
            )
            break
        ...
        DataV = np.concatenate(
            (Data[1:num_samples, :], Data[num_samples + 1 :, 1:]), axis=1
        )

        # Calculate range profile including calibration
        rp_iq_data = (
            2
            * np.fft.fft(DataV[:, :] * hann_window_2d, n=NFFT, axis=0)
            / sca_hann_window
            * Brd.FuSca
        )
        ...
        local_range_m, local_angle_deg, amplitude = compute_range_and_angle(
            normalized_amp, sigpro_cfg
        )

        if local_range_m is not None:
            engagement_zone_flag = check_engagement_zone(
                local_range_m, local_angle_deg, sigpro_cfg
            )

            # Perform coordinate transformation based on zone
            range_m, angle_deg = coord_transform(
                local_range_m, local_angle_deg, sigpro_cfg
            )
            logger.info(
                f"Range: {range_m:.4f} m, Azimuth: {angle_deg:.4f} deg, Engagement Zone: {engagement_zone_flag}"
            )
            logger.debug(
                f"Amplitude: {amplitude:.4f} dB, Zone Number: {sigpro_cfg.zone_number}, "
                "Local Range: {local_range_m:.4f} m, Local Angle: {local_angle_deg:.4f} deg"
            )

            # Send radar_report via DDS
            if sigpro_cfg.dds_enabled:
                sigpro_cfg.radar_report_writer.send(
                    range_m, angle_deg, sigpro_cfg.zone_number, engagement_zone_flag
                )
        else:
            logger.debug(
                f"No detections found. Average Amplitude: {np.average(normalized_amp):.4f}"
            )

        # Plots
        if plot_cfg.time_signals:
            plot_time_signals(DataV, num_channels, N)

        if plot_cfg.range_profile:
            plot_range_profile(rp_iq_data, sigpro_cfg)

        if plot_cfg.sum_data:
            plot_sum_data(rp_iq_data, sigpro_cfg)

        if plot_cfg.az_data:
            plot_az_data(normalized_amp, sigpro_cfg)

        if plot_cfg.heat_map:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "min_range", type=int, help="Minimum range (in meters) for RF Signal Processing"
    )
    parser.add_argument(
        "max_range", type=int, help="Maximum range (in meters) for RF Signal Processing"
    )
    parser.add_argument(
        "-a",
        "--az",
        required=False,
        help="Flip the sum data plot option to show azimuth versus amplitude",
        action="store_true",
    )
    parser.add_argument(
        "-l",
        "--log",
        required=False,
        help="Enable debug logging to the command line",
        action="store_true",
    )
    parser.add_argument(
        "-f",
        "--floor",
        required=False,
        help="Configurable noise floor / threshold",
        type=int,
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        ...
        "--dds",
        required=False,
        help="Enable DDS messages to be sent for radar detection reports",
        action="store_true",
    )
    ...
    if args.plot == "range_profile":
        plot_cfg.range_profile = True
    if args.plot == "time_signals":
        plot_cfg.time_signals = True
    if args.plot == "az_data":
        plot_cfg.az_data = True
    if args.plot == "sum_data":
        plot_cfg.sum_data = True
    if args.plot == "heat_map":
        plot_cfg.heat_map = True
