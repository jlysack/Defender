...
import Class.Computation as Computation
...
class Connection:
            ...
            if self.cOpened == -1 and Open == 1:
                if self.cDevType == "RadarLog" or self.cDevType == "TinyRad":
                    ...
                    if self.cUid > 0:
                        ...
                        uid = fromstring(
                            numpy.array(self.cUid, dtype=numpy.uint64).tostring(),
                            dtype="uint32",
                        )
                        Data[2] = uid[1]
                        Data[3] = uid[0]
        ...
        if self.cType == "Usb":
            ...
            if TxData > 0:
                ...
                try:
                    ...
                    if len(RxBytes) == ((LenRxData - 1) * 4):
                        ...
                        Ret = []
                        print(
                            "len(RxBytes) wrong: %d != %d"
                            % (len(RxBytes), (LenRxData - 1) * 4)
                        )
                except socket.timeout:
                    Ret = []
                    print("socket timed out")
            ...
            if self.hCon > -1:
                ...
                if (Err > 0) and (Len > 0):
                    ...
                    ((self.cArmCmdBegin <= Type) and (Type <= self.cArmCmdEnd))
                    or ((self.cFpgaCmdBegin <= Type) and (Type <= self.cFpgaCmdEnd))
                ) and (Len > 0):
                    # print("Elff 1")
                    Ret = self.cRadSocket.recv(int(4 * Len[0]))
    ...
    def OpenTcpIpVideoCom(self, Idx):
        if (self.cType == "RadServe") and (self.hConVideo[Idx] == -1):
            ...
            try:
                ...
                self.cRadVideoSocket[Idx].settimeout(
                    None
                )  # receive complete video frame at once
    ...
    def ReplayFile(
        ...
        compData = False
        if (
            stDataType == "RangeProfile"
            or stDataType == "RangeDoppler"
            or stDataType == "DetectionList"
            or stDataType == "TargetTracker"
        ):
            compData = True
                ...
                for VidIdx in range(0, NumVideo):
                    ...
                    if len(Ret) >= 7:
                        if Ret[0] > 0:
                            ...
                            if len(Ret) >= 8:
                                self.cVideoName[VidIdx] = (
                                    numpy.array(Ret[7:], dtype=numpy.uint32)
                                    .tostring()
                                    .decode("utf-8")
                                    .replace("\x00", "")
                                )
                    ...
                    for VidIdx in range(0, NumVideo):
                        ...
                        if len(Ret) >= 6:
                            if Ret[0] > 0:
                                ...
                                self.cVideoName[VidIdx] = (
                                    numpy.array(Ret[6:], dtype=numpy.uint32)
                                    .tostring()
                                    .decode("utf-8")
                                    .replace("\x00", "")
                                )
    ...
    def ConSetFileParam(self, stName, Val, DataType="STRING"):
        if self.cType == "RadServe":
            ...
            if DataType.upper() == "INT" or isinstance(Val, int):
                ...
                Data[int(2 + len(Name)) : int(2 + len(Name) + 2)] = fromstring(
                    valArray, dtype=numpy.uint32
                )
                ...
                for Idx in range(0, len(Name)):
                    Data[2 + Idx] = Name[Idx]
                Data[int(2 + len(Name))] = uint32(len(Val))
                # len of array
                for Idx in range(0, len(Val)):
                    Data[int(3 + len(Name) + Idx)] = uint32(Val[Idx])
            elif (DataType.upper() == "ARRAY64") or (
                DataType.upper() == "DOUBLE_ARRAY"
            ):
                ...
                Data[
                    int(3 + len(Name)) : int(3 + len(Name) + len(Val) * 2)
                ] = fromstring(numpy.ascontiguousarray(Val), dtype=numpy.uint32)
            else:
                Len = len(Val) % 4
                stVal = Val
                if Len != 0:
        ...
        if self.hConDat > -1:
            ...
            try:
                ...
                while RxBytesLen < Len:
                    RxBytesLen = RxBytesLen + self.cRadDatSocket.recv_into(
                        buf[RxBytesLen:]
                    )
        ...
        if len(varargin) > 0:
            ...
            if stVal == "BufSiz":
                if len(varargin) > 1:
                    ...
                    if self.cType == "RadServe":
                        Ret = self.CmdSend(
                            0, int("0x6108", 0), [uint32(self.cBufSiz)], 0
                        )
                        Ret = self.CmdRecv()
            elif stVal == "Mult":
                if len(varargin) > 1:
                ...
                if len(varargin) > 1:
                    ...
                    if self.cType == "RadServe":
                        Ret = self.CmdSend(
                            0, int("0x610B", 0), [uint32(self.cDataPortKeepAlive)], 0
                        )
                        Ret = self.CmdRecv()
                ...
                if len(varargin) > 1:
                    ...
                    if self.cType == "RadServe":
                        Ret = self.CmdSend(
                            0, int("0x610A", 0), [uint32(self.cUsbNrTx)], 0
                        )
                        Ret = self.CmdRecv()
        ...
        if len(varargin) > 0:
            ...
            if stVal == "Mult":
                if len(varargin) > 1:
                    ...
                    if self.cType == "RadServe":
                        Ret = self.CmdSend(
                            0, int("0x6109", 0), [uint32(self.cMult), 1], 0
                        )
                        Ret = self.CmdRecv()
        ...
        if self.cType == "RadServe" and len(varargin) > 0:
            ...
            if stVal == "CreateCmdLogFile":
                ...
                if not stName:
                    ...
                    Data[0] = 1
                    Data[1] = uint32(len(stName))
                    Data[2 : int(2 + len(stName) / 4)] = fromstring(
                        stName, dtype="uint32"
                    )
        ...
        if self.cType == "RadServe" and len(varargin) > 0:
            ...
            if stVal == "General":
                if len(varargin) > 3:
                    ...
                    Data = zeros(
                        int(4 + len(stPath) / 4 + len(Param) * 2), dtype="uint32"
                    )
                    Data[0] = uint32(0)  # level = 0
                    Data[1] = uint32(len(stPath))
                    Data[2 : int(2 + len(stPath) / 4)] = fromstring(
                        stPath, dtype="uint32"
                    )
                    ...
                    if len(Param) > 0:
                        ...
                        Data[
                            int(2 + len(stPath) / 4 + 2) : int(
                                2 + len(stPath) / 4 + 2 + len(Param) * 2
                            )
                        ] = fromstring(ParamStr, dtype=uint32)
        ...
        if self.cType == "RadServe" and len(varargin) > 0:
            ...
            if stVal == "List":
                ...
                if len(varargin) > 3:
                    ...
                    Data[1] = uint32(varargin[2])
                    Data[2] = uint32(len(stName))
                    Data[3 : int(3 + len(stName) / 4)] = fromstring(
                        stName, dtype="uint32"
                    )
                ...
                if len(varargin) > 6:
                    ...
                    Data[8 : int(8 + len(stFile) / 4)] = fromstring(
                        stFile, dtype="uint32"
                    )
                    Data[int(8 + len(stFile) / 4) :] = uint32(varargin[7])
                    # Raw
    ...
    def AddVideo(self, Add):
        print(
            "AddVideo is obsolete, please use RadServe_ListCameras() to receive camera list and RadServe_SelectCamera(<id>, <rate>, <name>) to select a camera"
        )
                ...
                if (self.cDataIdx) % self.cVideoRate[VidIdx] == 0:
                    ...
                    if len(inp) == self.cVideoSize[VidIdx]:
                        inp = reshape(
                            inp,
                            (
                                self.cVideoRows[VidIdx],
                                self.cVideoCols[VidIdx],
                                self.cVideoChn[VidIdx],
                            ),
                        )
        ...
        for VidIdx in range(0, len(self.hConVideo)):
            ...
            if self.hConVideo[VidIdx] > 0 and self.cVideoRate[VidIdx] != 0:
                if self.cExtSize == 0:
                    ...
                    if (
                        ((self.cDataIdx + 1) * self.cExtMult)
                        - self.cVideoFrmsColl[VidIdx] * self.cMult
                    ) / (self.cVideoRate[VidIdx] * self.cMult) >= 1:
                        readVid = True
                ...
                if readVid:
                    ...
                    if len(rdSock) == 1 and rdSock[0] == self.cRadVideoSocket[VidIdx]:
                        ...
                        if len(inp) == self.cVideoSize[VidIdx]:
                            self.cVideoFrmsColl[VidIdx] = (
                                self.cVideoFrmsColl[VidIdx] + 1
                            )
                            inp = reshape(
                                inp,
                                (
