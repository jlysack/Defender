...
import Class.Connection as Connection
...
class UsbADI(Connection.Connection):
    ...
    def Dsp_GetSwVers(self):
        ...
        dRet = {"SwPatch": -1, "SwMin": -1, "SwMaj": -1, "SUid": -1, "HUid": -1}
        if Vers[0] is True:
            Data = Vers[1]
            if len(Data) > 2:
                Tmp = Data[0]
    ...
    def BrdGetCalDat(self):
        ...
        CalDat[:] = (
            double(ConvDat[0:16:2]) / 2**24 + 1j * double(ConvDat[1:16:2]) / 2**24
        )
