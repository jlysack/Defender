...
class SigProConfig:
    def __init__(
        self, Brd, min_range, max_range, noise_floor=-35, zone=3, dds_enabled=False
    ):
        ...
        min_range_idx = np.argmin(
            np.abs(range_vector - min_range)
        )  # array index of min_range meters
        max_range_idx = np.argmin(
            np.abs(range_vector - max_range)
        )  # array index of RMax meters
        range_extent_vector = range_vector[
            min_range_idx:max_range_idx
        ]  # array of ranges between min_range and max_range
        ...
        angle_extent_vec = (
            np.arcsin(2 * np.arange(-NFFTAnt // 2, NFFTAnt // 2) / NFFTAnt)
            / np.pi
            * 180
        )
...
class PlotConfig:
    def __init__(self):
        ...
        self.sum_data = (
            False  # plot sum of all 7 range profile channels across all angles
        )
        self.az_data = (
            False  # plot sum of all 7 range profile channels across all range cells
        )
