...
class TinyRad(UsbADI.UsbADI):
    def __init__(self, stConType="Usb", *args):
        ...
        dUSpiCfg = dict()
        dUSpiCfg = {"Mask": 7, "Chn": self.Rf_USpiCfg_Rx_Chn}
        self.Adf_Rx = DevAdf5904.DevAdf5904(weakref.ref(self), dUSpiCfg)
        dUSpiCfg = dict()
        dUSpiCfg = {"Mask": 7, "Chn": self.Rf_USpiCfg_Tx_Chn}

        self.Adf_Tx = DevAdf5901.DevAdf5901(weakref.ref(self), dUSpiCfg)

        dUSpiCfg = dict()
        dUSpiCfg = {"Mask": 7, "Chn": self.Rf_USpiCfg_Pll_Chn}
    ...
    def RfMeas(self, dCfg):
        ...
        if (
            4 * dCfg["N"] * dCfg["FrmMeasSiz"] * dCfg["CycSiz"] * len(dCfg["Seq"])
        ) > int("0x60000", 16):
            print(
                "Required memory greater than DSP buffer size: decrease CycSiz, N, FrmMeasSiz"
            )
    ...
    def BrdGetData(self, NrPack=1, Extension=False):
        ...
        if self.cType == "Usb":
            ...
            if (
                (not Extension)
                and self.Computation.GetDataType() <= 1
                and (self.cReplayExt < 1)
            ):
