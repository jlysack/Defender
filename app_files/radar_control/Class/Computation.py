...
class Computation:
    ...
    def Plot(self, Data):
        if self.dataType <= 1:
            ...
            if numpy.count_nonzero(Data) == size(Data):
                for idxChn in range(0, self.nrChn):
                    self.viewCurves[idxChn].setData(
                        self.viewX[0:], 20 * log10(abs(Data[0, :, idxChn, 0]))
                    )

        elif self.dataType == 3:
            self.viewImg.setImage(
                20 * numpy.log10(numpy.abs(Data[:, :, 0, 0])) + 0.5e-6,
                pos=[self.viewY[0], self.viewX[0]],
                scale=[
                    (self.viewY[-1] - self.viewY[0]) / len(self.viewY),
                    (self.viewX[-1] - self.viewX[0]) / len(self.viewX),
                ],
            )
    ...
    def CreateView(self):
        ...
        if self.dataType <= 1:
            ...
            c0 = 3e8
            kf = (
                conn.GetFileParam("fStop") - conn.GetFileParam("fStrt")
            ) / conn.GetFileParam("TRampUp")
            ...
            vFreqVel = (
                numpy.linspace(-NFFTVel / 2, NFFTVel / 2 - 1, NFFTVel)
                / NFFTVel
                / conn.GetFileParam("Tp")
            )
    ...
    def SetType(self, stType):
        ...
        if stType == self.RP:
            if (
                self.isSet_FuSca
                and self.isSet_fStrt
                and self.isSet_fStop
                and self.isSet_TRampUp
                and self.isSet_fs
                and self.isSet_CalRe
                and self.isSet_CalIm
            ):
                id = 2
    ...
    def GetData(self, NrPack):
        ...
        if self.dataType == 2:
            ## range profile
            rpData = conn.ConGetData(
                NrPack * self.rpLen * self.nrChn * self.dtypeLen * 2
            )
            if rpData == []:
                return rpData
            ...
            if NrPack == 1:
                ...
                if self.rpSort > 0:
                    Data = cData.reshape(NrPack, int(self.nrChn), self.rpLen).transpose(
                        1, 2, 0
                    )
                else:
                    Data = cData.reshape(NrPack, int(self.nrChn), self.rpLen).transpose(
            ...
            ## range doppler
            rdData = conn.ConGetData(
                NrPack * self.rpLen * self.rdLen * self.nrChn * self.dtypeLen * 2
            )
            rdData.dtype = "float32"
            cData = rdData[0::2] + 1j * rdData[1::2]
            if NrPack == 1:
                Data = cData.reshape(int(self.nrChn), self.rpLen, self.rdLen).transpose(
                    2, 1, 0
                )
            else:
                Data = cData.reshape(
                    int(self.nrChn), self.rpLen, self.rdLen, NrPack
                ).transpose(2, 1, 0, 3)
            ...
            dlData = conn.ConGetData(
                NrPack
                * self.dlLen
                * (5 * self.dtypeLen + 2 * self.dtypeLen * self.nrChn)
            )
            ...
            for tIdx in range(1, NrPack * self.dlLen):
                ...
                Target["Amp"] = (
                    dlData[
                        int(5 + Offset * (tIdx - 1)) : int(
                            5 + 2 * self.nrChn + Offset * (tIdx - 1)
                        ) : 2
                    ]
                    + 1j
                    * dlData[
                        int(6 + Offset * (tIdx - 1)) : int(
                            6 + 2 * self.nrChn + Offset * (tIdx - 1)
                        ) : 2
                    ]
                )
                if not (
            ...
            len = self.dtypeLen * (
                3
                + (9 + 2 * self.ttNumHist) * self.ttNumTracks
                + self.ttTarSize * self.ttNumTar
            )

            ttData = conn.ConGetData(NrPack * len)
            ttData.dtype = "float32"
            ttData = ttData[3:]
            # ignore first three entries, as they contain the LenHist, Target count and TarSize values

            Tracks = []
            for trackIdx in range(0, self.ttNumTracks):
                Track = dict(
                    Id=fromstring(
                        ttData[int((9 + 2 * self.ttNumHist) * trackIdx + 0)].tostring(),
                        dtype="int32",
                    )[0],
            ...
            for tarIdx in range(0, self.ttNumTar):
                if self.ttTarSize == 2:
                    ...
                    Det = dict(
                        X=ttData[int(self.ttTarSize * tarIdx)]
                        * sin(ttData[int(3 + self.ttTarSize * tarIdx)]),
                        Y=ttData[int(self.ttTarSize * tarIdx)]
                        * cos(ttData[int(3 + self.ttTarSize * tarIdx)]),
                        ...
                        Track=fromstring(
                            ttData[int(5 + self.ttTarSize * tarIdx)].tostring(),
                            dtype="int32",
                        )[0],
                        Amp=[],
                    )
                    Det["Amp"] = (
                        ttData[
                            int(6 + self.ttTarSize * tarIdx) : int(
                                6 + 2 * self.nrChn + self.ttTarSize * tarIdx
                            ) : 2
                        ]
                        + 1j
                        * ttData[
                            int(7 + self.ttTarSize * tarIdx) : int(
                                7 + 2 * self.nrChn + self.ttTarSize * tarIdx
                            ) : 2
                        ]
                    )
                    if not (
