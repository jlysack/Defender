...
async def main_execution_loop():
    ...
    while True:
        ...
        # If commanded zone is same as current/previous, do nothing
            ...
            logger.info(
                "Commanded zone is same as current zone. Bypassing stepper activation."
            )
        ...
        if radar_control.check_radar_safety_file() == "0":
            logger.info(
                "Radiation Disabled via Safety Toggle. Resend Scan Instruction after enabling radiation safety toggle."
            )
            continue
        ...
        logger.info(
            f"Zone {zone} - Minimum range: {min_range} m, Maximum range: {max_range} m"
        )

        # Instantiate TinyRad and sigpro_cfg objects
        Brd = radar_control.configure_tinyrad()
        sigpro_cfg = SigProConfig(
            Brd, min_range, max_range, const.DEFAULT_NOISE_FLOOR, zone, dds_enabled
        )
        ...
        radar_process = Process(
            target=radar_control.radar_search,
            args=(Brd, sigpro_cfg, plot_cfg, process_queue),
        )
        radar_process.start()
